...
import br.org.ici.saude.geradorcodigo.angular.models.PesquisaComponentModel;
...
public class GeradorPesquisaComponentAngularRequest implements GeradorAngularArquivo {
        ...
        .map(entidadeArq -> {
          ...
          Collection<? extends AtributosModel> atributosFiltratos = filtroAtributos
              ...
              .map(atr -> new AtributosAngularModel(atr.getNome(), atr.getMensagem(), atr.getTipo(),
                  atr.getAnotacoes()))
              .toList();
          return new PesquisaComponentModel(entidadeArq.getNome(),
