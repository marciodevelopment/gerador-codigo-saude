...
public class GeradorGetAngularResponse implements GeradorAngularArquivo {

  public List<? extends ArquivoAngularModel> converterParaArquivoModel(
      ArquivoConfiguracao arquivoConfiguracao) {
    return arquivoConfiguracao.getEntidades().stream().filter(EntidadeArquivo::existeGet)
        .map(entidadeArq -> {
          ...
          Collection<AtributosModel> atributosFiltratos = filtroAtributos
              .getAtributosDesnormalizadosModel(entidadeArq.getNome(), MetodoType.GET);
          ...
          return new InterfaceAngularModel(ArquivoAngularType.GET_RESPONSE, entidadeArq.getNome(),
